...
int GetGATE_PTEADDR(int *x)  { return(x[GATE_PTEADDR]); }
int GetGATE_PA(int *x)       { return(x[GATE_PA]); }
int GetCHK_EXC(int *x)       { return(x[CHK_EXC]); }
	...
	*/
	...
	if(CURRENT_LATCHES.NEXT > 0)
	{
		...
		*/
		...
		else if(GetCHK_EXC(CURRENT_LATCHES.MICROINSTRUCTION))
		{
			if((CURRENT_LATCHES.IR >> 12) != 15)
			{
				if(((CURRENT_LATCHES.MDR >> 3) & 0x0001) == 0)
				{
					if(DEBUG1)
					{
						printf("INTERRUPT SET. PROTECTION EXCEPTION.\n");
			...
			else if(((CURRENT_LATCHES.MDR >> 2) & 0x0001) == 0)
			{
				if(DEBUG1)
				{
					printf("INTERRUPT SET. PAGE FAULT EXCEPTION.\n");
				}
				INT = 1;
				NEXT_LATCHES.INTV = 0;
				NEXT_LATCHES.EXCV = 0x02;
	...
	if(GetIRD(CURRENT_LATCHES.MICROINSTRUCTION) == 1)
	{
		...
		switch(GetCOND(CURRENT_LATCHES.MICROINSTRUCTION))
		{
			...
			case 3:
			{
				memcpy(NEXT_LATCHES.MICROINSTRUCTION, CONTROL_STORE[TRANSLATE] ,sizeof(int)*CONTROL_STORE_BITS);
				NEXT_LATCHES.STATE_NUMBER = TRANSLATE;
				break;
			}
			/*
			default:
...
void eval_bus_drivers() {
   ...
   */    
	...
	GATE_PTEADDR_INPUT = CURRENT_LATCHES.PTBR + ((CURRENT_LATCHES.VA & 0xFE00) >> 8);
	if(DEBUG3)
	{
		printf("Gate_PTEADDR_INPUT: %x\n", GATE_PTEADDR_INPUT);
	}
	GATE_PA_INPUT = (CURRENT_LATCHES.MDR & PTE_PFN_MASK) + (CURRENT_LATCHES.VA & 0x01FF);
...
void drive_bus() {
   ...
   */       
	...
	else if(GetGATE_PTEADDR(CURRENT_LATCHES.MICROINSTRUCTION))
	{
		BUS = Low16bits(GATE_PTEADDR_INPUT);
	}
	else if(GetGATE_PA(CURRENT_LATCHES.MICROINSTRUCTION))
	{
		BUS = Low16bits(GATE_PA_INPUT);
	}
	else
	{
		BUS = 0;
...
void latch_datapath_values() {
   ...
   */       
	...
	{
		if(((CURRENT_LATCHES.IR >> 12) == 3) || ((CURRENT_LATCHES.IR >> 12) == 7)) 
		{
